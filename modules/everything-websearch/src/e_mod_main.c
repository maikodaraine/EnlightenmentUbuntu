...
_send_request(void *data)
{
   ...
   if (!strcmp(p->base.name, "Translate"))
...
_youtube_dl_finish(Youtube_Data *yd, int abort)
{
   ...
   if (abort)
     {
	_send_notification(yd->id, "emblem-sound", _("Abort download"),
			   yd->label, -1);
...
_youtube_dl_timer(void *d)
{
   ...
   if (stat(yd->filepath, &s) == 0)
	...
	if (s.st_size < 262144)
	  {
	     ...
	     char buf[128];
	     snprintf(buf, sizeof(buf), _("Got %d kbytes of"),
	...
	if ((yd->method == YOUTUBE_DL_ENQ) &&
	    (act = evry->action_find("Enqueue in Playlist")))
	  {
	     act->it1.item = EVRY_ITEM(f);
	     act->action(act);
	     _send_notification(yd->id, "emblem-sound", _("Enqueue"), yd->label, -1);
	  }
	if ((yd->method == YOUTUBE_DL_PLAY) &&
	    (act = evry->action_find("Play File")))
	  {
	     act->it1.item = EVRY_ITEM(f);
	     act->action(act);
	     _send_notification(yd->id, "emblem-sound", _("Play"), yd->label, -1);
	  }
	IF_RELEASE(f->path);
	IF_RELEASE(f->mime);
	E_FREE(f);
...
_youtube_dl_cb_del(void *data, int type __UNUSED__, void *event)
{
   ...
   _send_notification(yd->id, "emblem-sound", _("Finished download"),
		      yd->label, -1);
...
_youtube_dl_data_cb(Url_Data *dd)
{
   ...
   if (yd->method == YOUTUBE_PLAY)
	...
	_send_notification(yd->id, "emblem-sound", _("Start download"), yd->label, -1);
     }
...
_youtube_dl_action(Evry_Action *act)
{
   ...
   if (((method == YOUTUBE_DL) ||
	(method == YOUTUBE_DL_ENQ)) &&
       (youtube_dl_active > 1))
     {
	_send_notification(1337, "emblem-sound", _("Enqueue files for download"), "", -1);
...
_upload_data(Url_Data *dd)
{
   ...
   if (d)
	...
	_send_notification(ud->id, "image", _("Upload Image"), _("Link copied to clipboard") , -1);
	_open_url(d->value);
     }
   else
     {
	_send_notification(ud->id, "image", _("Upload Image"), _("Something went wrong :("), -1);
     }
...
_upload_progress(Url_Data *dd, Ecore_Con_Event_Url_Progress *ev)
{
   ...
   if ((int) up > ud->progress)
	...
	ud->progress = up;
	snprintf(buf, sizeof(buf), _("Completed %1.1f%% of %s"), up * 5.0, ud->file);

	_send_notification(ud->id, "image", _("Upload Image"), buf, -1);
     }
...
_action_upload(Evry_Action *act)
{
   ...
   _send_notification(ud->id, "image", _("Upload Image"), ud->file, -1);
